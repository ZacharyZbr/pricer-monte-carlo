...
void MonteCarlo::paralleldelta(PnlVect *delta, PnlVect *std_dev)
{
    ...
    for (int d = 0; d < opt_->size_; d++){
        double ksi_carre = (1/(0.1*0.1*2*2))*(exp(-2 * mod_->r_* opt_->T_) * 1/nbSamples_ * pnl_vect_get(meanPayoffSquared, d) - (delta->array[d]/nbSamples_)*(delta->array[d]/nbSamples_) * exp(-2 * mod_->r_* opt_->T_));
        pnl_vect_set(std_dev, d, sqrt(ksi_carre/nbSamples_));
    }
